...
public class ShopController {
    ...
    private Map<String, Object> listShops(HttpServletRequest request) {
        ...
        if (pageIndex > 0 && pageSize > 0) {
            ...
            try {
                ...
                logger.debug("获取店铺列表失败"+e.getMessage());
                modelMap.put("success", false);
                modelMap.put("errMsg", e.toString());
                return modelMap;
            }
    ...
    private Map<String, Object> searchShopById(HttpServletRequest request) {
        ...
        if (shopId > 0) {
            try {
                ...
                logger.debug("店铺信息"+e.getMessage());
                modelMap.put("success", false);
                modelMap.put("errMsg", e.toString());
                return modelMap;
            }
    ...
    private Map<String, Object> modifyShop(String shopStr, HttpServletRequest request) {
        ...
        try {
            ...
            logger.debug("转换成shop实例失败"+e.getMessage());
            modelMap.put("success", false);
            modelMap.put("errMsg", e.toString());
            return modelMap;
        }
        ...
        if (shop != null && shop.getShopId() != null) {
            try {
                ...
                logger.debug("修改店铺信息失败错误"+e.getMessage());
                modelMap.put("success", false);
                modelMap.put("errMsg", e.toString());
                return modelMap;
            }
