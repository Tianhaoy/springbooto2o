...
public class UserAwardManagementController {
    private static Logger logger = LoggerFactory.getLogger(UserAwardManagementController.class);
    @Autowired
    ...
    private Map<String, Object> listUserAwardMapsByShop(HttpServletRequest request) {
        logger.debug("开始列出某个店铺的用户奖品领取情况列表---");
        ...
        if ((pageIndex > -1) && (pageSize > -1) && (currentShop != null) && (currentShop.getShopId() != null)) {
            ...
            logger.debug("店铺信息为"+currentShop);
            // 从请求中获取奖品名
            String awardName = HttpServletRequestUtil.getString(request, "awardName");
            logger.debug("奖品名"+awardName);
            if (awardName != null) {
            ...
            logger.debug("某个店铺的用户奖品领取情况列表"+ue.getUserAwardMapList());
    ...
    private String exchangeAward(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (auth != null) {
            logger.debug("获取负责扫描二维码的店员信息"+auth);
            // 通过userId获取店员信息
    ...
    private WechatAuth getOperatorInfo(HttpServletRequest request) {
        ...
        if (null != code) {
            ...
            try {
                ...
                logger.debug("获取扫描二维码的店员信息为"+auth);
            } catch (IOException e) {
                logger.debug(e.getMessage());
                e.printStackTrace();
            }
    ...
    private UserAwardMap compactUserAwardMap4Exchange(Long customerId, Long userAwardId, PersonInfo operator) {
        logger.debug("开始封装用户奖品映射实体类，以供扫码使用，主要将其领取状态变为已领取---");
