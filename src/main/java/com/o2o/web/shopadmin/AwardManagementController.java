...
public class AwardManagementController {
    private static Logger logger = LoggerFactory.getLogger(AwardManagementController.class);
    @Autowired
    ...
    private Map<String, Object> listAwardsByShop(HttpServletRequest request) {
        logger.debug("开始通过店铺id获取该店铺下的奖品列表---");
        ...
        if ((pageIndex > -1) && (pageSize > -1) && (currentShop != null) && (currentShop.getShopId() != null)) {
            logger.debug("shopId为"+currentShop.getShopId());
            // 判断查询条件里面是否传入奖品名，有则模糊查询
            String awardName = HttpServletRequestUtil.getString(request, "awardName");
            logger.debug("奖品名为"+awardName);
            // 拼接查询条件
            ...
            logger.debug("获取奖品列表为"+ae.getAwardList());
    ...
    private Map<String, Object> getAwardbyId(HttpServletRequest request) {
        logger.debug("开始通过商品id获取奖品信息---");
        Map<String, Object> modelMap = new HashMap<String, Object>();
        // 从request里边获取前端传递过来的awardId
        long awardId = HttpServletRequestUtil.getLong(request, "awardId");
        logger.debug("awardId为"+awardId);
        // 空值判断
        if (awardId > -1) {
            // 根据传入的Id获取奖品信息并返回
